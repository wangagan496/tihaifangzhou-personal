...
本项目早期基于老师指导下完成的基础版本，当前仓库用于记录本人后续的功能完善、重构和优化工作。

当前仓库由现阶段源代码快照重新初始化，不包含早期协作阶段的 Git 提交历史；这仅用于独立记录后续维护，不改变早期协作事实。原始协作仓库为 [`wangagan496/tihaifangzhou`](https://github.com/wangagan496/tihaifangzhou)。分支结构与本地一致：`full` 为完整应用，`main` 为基础框架，其余为按功能拆分的学习分支，详见 [分支说明](docs/BRANCHES.md)。
...
朗读、卡片与评分的接入说明和接口契约见 [学习能力接入说明](docs/STUDY_INTEGRATIONS.md)。
独立评分服务的启动、配置与边界见 [后端说明](backend/README.md)。
...
当前源码于 2026-09-13 使用本机 `6.1.1(24)` 工具链完成以下验证：
